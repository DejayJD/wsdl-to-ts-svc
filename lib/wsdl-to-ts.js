...
function outputTypedWsdl(a) {
    ...
    for (const service of Object.keys(a.files)) {
        for (const port of Object.keys(a.files[service])) {
            ...
            const interfaceFile = { file: fileName + "Types", data: [] };
            const serviceFile = { file: fileName, data: [] };
            ...
            if (a.methods[service] && a.methods[service][port]) {
                const ms = [];
                serviceFile.data.push(templates_1.default.serviceHeaderTemplate({ serviceName: service }));
                for (const method of Object.keys(a.methods[service][port])) {
                    ...
                    serviceFile.data.unshift(templates_1.default.serviceImportTemplate(templateObj));
                    serviceFile.data.push(templates_1.default.serviceMethodTemplate(templateObj));
                }
